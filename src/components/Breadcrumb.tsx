        ...
        <ol className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
          {items.map((item, i) => (
            <li key={i} className="flex items-center gap-2">
              {item.path ? (
                <Link href={item.path} className="hover:text-primary-wine transition-colors">
                  {item.name}
                </Link>
              ) : (
                <span className="text-primary-wine font-semibold">{item.name}</span>
              )}
              {i < items.length - 1 && <ChevronRight size={12} className="text-gray-400" />}
            </li>
          ))}
        </ol>
      </nav>
